...
export default class NoticeIcon extends PureComponent {
  ...
  onViewEdit = (tabProps, event) => {
    const { onViewEdit } = this.props;
    onViewEdit(tabProps, event);
  };
  ...
  getNotificationBox() {
    ...
    const panes = React.Children.map(children, child => {
      const { list, title, count, emptyText, emptyImage, showClear, showViewEdit } = child.props;
      ...
      return (
        <TabPane tab={tabTitle} key={title}>
          <List
            ...
            onViewEdit={event => this.onViewEdit(child.props, event)}
            showClear={showClear}
